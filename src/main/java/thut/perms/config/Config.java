...
public class Config
{
    public static abstract class ConfigData implements IConfigHolder
    {
        ...
        {
            Perms.LOGGER.debug("{} config belongs to us!", configEvent.getConfig().getFileName());
        ...
        {
            Perms.LOGGER.info("Loaded {} config file {}", this.MODID, configEvent.getConfig().getFileName());
            this.read(configEvent.getConfig());
        }
        ...
        {
            Perms.LOGGER.info("Reading {}", config.getFileName());
            ...
            for (final Field f : values.keySet())
                try
                {
                    ...
                    Perms.LOGGER.info("Set {} to {}", f.getName(), o);
                    f.set(this, o);
                    changed = true;
                }
                catch (final Exception e)
                {
                    Perms.LOGGER.error("Error updating config value for " + f, e);
                }
            return changed;
        }
        ...
        private boolean write(final ModConfig config, final Map<Field, ConfigValue<?>> values)
        {
            ...
            for (final Field f : values.keySet())
                ...
                {
                    Perms.LOGGER.error("Error saving config value for " + f, e);
                }
            return ret;
        }
    }
